...
function html5Upload() {
	...
	sUploadURL = '/blog/review/imgUpload'; // upload URL
 ...
 *     :"http://static.naver.net/www/u/2010/0611/nmms_215646753.gif", bNewLine : true }, { sFileName :
 *     "btn_sch_over.gif", sFileURL :"http://static1.naver.net/w9/btn_sch_over.gif", bNewLine : true } ]
 */
...
jindo.$Ajax.prototype.request = function(oData) {
	...
	if (req.addEventListener && !(navi.indexOf("Opera") > -1) && !(navi.indexOf("MSIE") > -1)) {
		/*
		 * opera 10.60에서 XMLHttpRequest에 addEventListener기 추가되었지만 정상적으로 동작하지 않아 opera는 무조건 dom1방식으로
		 * 지원함. IE9에서도 opera와 같은 문제가 있음.
		 */
		if (this._loadFunc) {
		...
		if (typeof req.onload != "undefined") {
			...
			/*
			 * IE6에서는 onreadystatechange가 동기적으로 실행되어 timeout이벤트가 발생안됨. 그래서 interval로 체크하여 timeout이벤트가
			 * 정상적으로 발생되도록 수정. 비동기 방식일때만
			 * 
			 */
