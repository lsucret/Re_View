function createSEditor2(elIRField, htParams, elSeAppContainer) {
	...
	oEditor.registerPlugin(new nhn.husky.SE2M_EditingModeChanger(elAppContainer, htConversionMode)); // 모드간
																																		// 변경(Editor,
																																		// HTML,
																																		// Text)
	...
	oEditor.registerPlugin(new nhn.husky.SE_EditingAreaVerticalResizer(elAppContainer, htConversionMode)); // 편집영역
																																				// 리사이즈
	...
	oEditor.registerPlugin(new nhn.husky.SE2M_QuickEditor_Common(elAppContainer)); // 퀵에디터 공통(표,
																												// 이미지)
	...
	oEditor.registerPlugin(new nhn.husky.SE2M_Accessibility(elAppContainer)); // 에디터내의 웹접근성 관련 기능모음
																										// 플러그인
